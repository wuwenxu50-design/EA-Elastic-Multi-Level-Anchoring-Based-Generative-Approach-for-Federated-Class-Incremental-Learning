    ...
    def _server_update_anchor2_pool(self, client_anchor2_updates):
        ...
        for y, arr in per_class.items():
            ...
            if X.shape[0] >= K:
                ...
                centers = self._kmeans_torch(X, K, iters=self.kmeans_iter)  # 会自动 pad
    ...
    def _local_finetune(self, teacher, model, train_data_loader, task_id, client_id, lr):
        ...
        for it in range(self.args["local_ep"]):
            ...
            for batch_idx, (_, images, labels) in iter_loader:
                ...
                if isinstance(syn_batch, (tuple, list)):
                    if len(syn_batch) == 3:
                        _, syn_input, _ = syn_batch
                    elif len(syn_batch) >= 1:
                        syn_input = syn_batch[0]
                    else:
                        raise RuntimeError("Empty syn_batch from syn_data_iter")
                else:
                    syn_input = syn_batch
