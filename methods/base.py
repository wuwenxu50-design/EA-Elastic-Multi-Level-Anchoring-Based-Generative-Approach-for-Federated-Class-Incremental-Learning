...
class BaseLearner(object):
    ...
    def _compute_accuracy(self, model, loader,log_path=None):
        model.eval()
        correct, total = 0, 0
        writer = None
        fobj = None
        if log_path is not None:
            ...
            fobj = open(log_path, "w", newline="")
            writer = csv.writer(fobj)
            writer.writerow(["batch_idx", "batch_size", "batch_correct", "batch_acc(%)"])
        for i, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self.args["gpu"])
            with torch.no_grad():
                outputs = model(inputs)["logits"]
            predicts = torch.max(outputs, dim=1)[1].cpu()
            targets_cpu = targets.cpu()
            batch_correct = (predicts == targets_cpu).sum().item()
            batch_size = len(targets_cpu)
            batch_acc = 100.0 * batch_correct / max(1, batch_size)
            correct += batch_correct
            total += batch_size
            if writer is not None:
                writer.writerow([i, batch_size, batch_correct, round(batch_acc, 4)])
        if fobj is not None:
            fobj.close()

        return np.around((correct * 100.0) / max(1, total), decimals=2)
