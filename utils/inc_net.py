...
class IncrementalNet(BaseNet):
    ...
    def radius_hat(self) -> torch.Tensor:
        assert self.radius_raw is not None, "radius_raw is not initialized"
    ...
    def update_fc(self, nb_classes):
        ...
        del self.fc
        self.fc = fc.cuda()
        if int(self.args.get('use_soft_radius', 0)) == 1:
            device = self.fc.weight.device
            init_r = float(self.args.get('r_init', self.args.get('r', 0.015)))
            init_raw = self._inv_softplus(torch.tensor(init_r, device=device, dtype=torch.float32))

            if self.radius_raw is None:
                self.radius_raw = nn.Parameter(init_raw.repeat(nb_classes))
            else:
                old = self.radius_raw.data
                if old.numel() < nb_classes:
                    new = init_raw.repeat(nb_classes)
                    new[: old.numel()] = old
                    self.radius_raw = nn.Parameter(new)
